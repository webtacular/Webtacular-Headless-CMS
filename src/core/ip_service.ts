...
import { ErrorInterface, IPhistoryInterface, IPobjectSettingsInterface } from './interfaces';
import { mongoDB } from './db_service';
import { getTimeInSeconds } from './general_service';
import { locals } from './response_handler';

/**
 * Gets the ip from a fastify request object, parses it and than returns it as B64
 * 
 * @param req Fastify request object
 * @returns string - Base64 encoded IP
 */
export function getIP(req:any):string {
    let ip = Object.keys(req.headers).includes('cf-connecting-ip') === false ? req.ip : req.headers['cf-connecting-ip'];
...
let log_new = async (ip:string, id:ObjectId, returnError?:boolean, config?:IPobjectSettingsInterface):Promise<IPhistoryInterface | boolean | ErrorInterface> => {
    return new Promise((resolve, reject) => {
        ...
        let ipOBJ:IPhistoryInterface = {
            ...
            ip,
            settings: {
                bypass_account_limit: config?.bypass_account_limit ?? false,
                bypass_timeout: config?.bypass_timeout ?? false,
            },
            accounts: [
                {
                    user_id: id,
...
let log_old = async (ip:string, id:ObjectId, ip_history:IPhistoryInterface, returnError?:boolean, config?:IPobjectSettingsInterface):Promise<IPhistoryInterface | boolean | ErrorInterface> => {
    return new Promise((resolve, reject) => {
        ...
        Object.assign(ip_history, {
            ...
            settings: {
                bypass_account_limit: config?.bypass_account_limit ?? ip_history.settings.bypass_account_limit, 
                bypass_timeout: config?.bypass_timeout ?? ip_history.settings.bypass_timeout,
            },
